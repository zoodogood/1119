...
async function commandHundler(msg){
  ...
  const checkAvailable = () => {
    const problems = [];
    if (cmd.type === "delete" || cmd.type === "dev" && msg.author.id !== "416701743733145612"){
...
Math.math = function(regular){
 ...
 let args = regular.match(/(\d+|\/|\*+|\+|-|%)/g);
  ...
  const priorities = ["**", "*|/", "+|-", "%"];
	while (true){
    if (!priorities[0] || !args[1]) return args.join(" ");
...
const commands = {
  ...
  commandinfo: new Command(async (msg, op) => {
    ...
    let embed = {
      ...
      image: gifURL || (__inServer ? null : "https://overclockers.ru/st/legacy/blog/151718/236757_O.jpg"),
  ...
  invites: new Command(async (msg, op) => {
    let message = msg.msg("Меню ");
  }, {dm: true, type: "guild"}, "приглашения")
}
...
const timeEvents = {
  day_stats: function (isLost){
    ...
    client.guilds.cache.filter(e => e.data.professions).each(guild => {
      ...
      guild.logSend(`Были выданы зарплаты`, {description: `С казны было автоматически списано ${ending(costs, "коин", "ов", "", "а")} на зароботные платы пользователям\nИх список вы можете просмотреть в команде \`!банк\`\nУчастников получило коины: ${workers.size}`});
