...
class Command extends BaseCommand {
  options = {
    ...
    media: {
      ...
      example: `!role <memb> <roleID> #аргументы нужны только при выдаче / снятии роли. roleID Даже в этом случае необязателен`,
    },
    accessibility: {
  ...
  rolesListReactionsCallbacks = {
    ...
    "⭐": async (context) => {
      ...
      _questionMessage = await interaction.channel.msg({
        title: "Укажите айди роли",
        description:
          "Она сможет выдавать или снимать участникам далее указанные роли",
      });
      const controllerId = (
        await interaction.channel.awaitMessage({
      ...
      _questionMessage = await interaction.channel.msg({
        title: "С чём связать..?",
        description: `Через пробел укажите айди всех ролей, которыми будет управлять ${controllerRole.name}`,
      });
      const rolesIds = (
        await interaction.channel.awaitMessage({
          user: interaction.user,
      ...
      tieRoles[controllerId] = [
        ...new Set(previousRoles.concat(rolesList.map((role) => role.id))),
      ];

      interaction.channel.msg({
        title: `Успешно добавлено: ${Util.ending(
          tieRoles[controllerId].length - previousRoles.length,
          "связ",
          "ей",
          "ь",
          "и",
    ...
    "❌": async (context) => {
      ...
      const _questionMessage = await interaction.channel.msg({
        ...
        description: `Вы очистите все связи с ролью ${rolesCache.get(
          controllerId,
        )}`,
      });
      const react = await _questionMessage.awaitReact(
  ...
  async displayUserRolesInterface({ user, interaction }) {
    ...
    if (!roleId) {
      ...
      const contents = {
        list: controledRoles
          .slice(0, numberReactions.length)
          .map(
            (id, i) =>
              `${numberReactions[i]} ${interaction.guild.roles.cache
                .get(id)
                ?.toString()}`,
          )
          .join("\n"),
  ...
  async onChatInput(msg, interaction) {
    const heAccpet = await Util.awaitUserAccept({
      ...
      message: {
        title:
          "С помощью этой команды администраторы серверов могут дать своим модераторам возможность выдавать или снимать определенные роли, не давая создавать новые или управлять старыми",
      },
  ...
  rolesListCreateEmbed(context) {
    ...
    const tieToPage = ([controlId, roles]) =>
      `[${String(rolesCache.get(controlId))}]\n${roles
        .map((id) => `• ${String(rolesCache.get(id))}`)
        .join("\n")}`;
    ...
    const embed = {
      ...
      footer: {
        text: `Чтобы выдать пользователю роль, используйте !роль @упоминание\n${
          userIsAdmin
            ? "С помощью реакций ниже создайте новую связь или удалите старые."
            : ""
  ...
  async rolesListOnReact(context, reaction, user) {
    ...
    context.pagesCount = Object.keys(
      this.getTieRoles(interaction.guild),
    ).length;
