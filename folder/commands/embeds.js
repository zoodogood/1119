...
class Command extends BaseCommand {
  async onChatInput(msg, interaction) {
    ...
    let embeds = await msg.channel.messages.fetch({
      limit: 100,
      before: interaction.params || null,
    });
    embeds.concat(
      await msg.channel.messages.fetch({
        limit: 100,
        before: embeds.last().id,
      }),
    );
    ...
    embeds = [
      ...embeds
        .filter((e) =>
          e.embeds.find((e) => e.type == "rich" && e.color != 10092543),
        )
        .values(),
    ];
    ...
    let input = embeds
      .reverse()
      .map((e, i) => i + 1 + ". " + e.embeds[0].title)
      .join("\n");
    const bot_msg = await msg.msg({
      title: "	• " + embeds.length + " <a:diamond:725600667586134138>",
      description: input,
      color: "#99ffff",
    });

    const eventFuncDelete = (e) => {
      if (
        e.channel.id != msg.channel.id ||
        !e.embeds.find((e) => e.type == "rich" && e.color != 10092543)
      )
        return;
      const index = embeds.findIndex((el) => el.id == e.id);
      if (!index) return;
      ...
      switch (react) {
        case "754777124413505577":
          answer = await msg.channel.awaitMessage(msg.author, {
            title: "Введите индекс элемента для получения его в JSON виде",
            embed: { color: "#99ffff" },
          });
          if (!answer) return;
          ...
          const element = embeds[answer - 1];
          msg.author.msg({
            title: "> " + element.embeds[0].title,
            description:
              "```JSON\n" +
              Discord.escapeCodeBlock(
                JSON.stringify(element.embeds[0], null, 2),
              ) +
              "```",
          });
          msg.msg({
        ...
        case "754780992023167007":
          answer = await msg.channel.awaitMessage(msg.author, {
            title:
              "Чтобы переместить сообщение, введите его позицию и место на которое его нужно переместить",
            embed: { color: "#99ffff" },
          });
          const indices = answer.content.match(/\d+/g);
          if (!indices[1]) {
            msg.msg({
              title: "Некорректное значение",
              description:
          ...
          embeds.splice(
            indices[0] - 1,
            1,
            ...embeds.splice(indices[1] - 1, 1, embeds[indices[0] - 1]),
          );
