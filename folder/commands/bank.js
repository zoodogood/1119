...
class Command {
    ...
    const cash = async (coins, isPut, cause) => {
      ...
      if (isPut){
        ...
        server.coins += coins;
        msg.guild.logSend({title: "Содержимое банка изменено:", description: `${interaction.member.displayName} отнёс в казну ${Util.ending(coins, "коин", "ов", "", "а")}`, footer: {iconURL: msg.author.avatarURL(), text: msg.author.tag}});
        msg.react("👌");
      ...
      if (!isPut){
        ...
        interaction.userData.coins += coins;
        server.coins -= coins;
        msg.guild.logSend({title: "Содержимое банка изменено:", description: `${interaction.member.displayName} обналичил казну на сумму **${ Util.ending(coins, "коин", "ов", "а", "ов")}**\nПричина: ${cause}`, footer: {iconURL: msg.author.avatarURL(), text: msg.author.tag}});
        msg.react("👌");
