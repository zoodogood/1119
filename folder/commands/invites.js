...
class Command extends BaseCommand {
  async onChatInput(msg, interaction) {
    if (interaction.mention) {
      ...
      const byInvitesCountContent = `За время пребывания бота на сервере, упомянутый пользователь пригласил ${Util.ending(
        invitesCount,
        "человек",
        "",
        "",
      ...
      const byGuildDataContent =
        guildInvites && guildInvites.size
          ? `${member.displayName} создал(-a) ${Util.ending(
              guildInvites.size,
              "",
              "ссыллок-приглашений",
              "ссылку-приглашение",
              "ссылки-приглашения",
            )} — посетило ${Util.ending(
    ...
    const answer = await Util.awaitUserAccept({
