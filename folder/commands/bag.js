  ...
  async onChatInput(msg, interaction) {
    ...
    const ITEMS = [
      ...
      },
      {
        ...
        ending: (count) =>
          `<:crys2:763767958559391795> ${Util.ending(
            count,
            "Опыт",
            "а",
            "",
      ...
      },
      {
        ...
        ending: (count) =>
          `🌱 ${Util.ending(count, "Сем", "ян", "ечко", "ечка")}`,
      },
      {
        key: "cheese",
    ...
    const moveItem = (key, count, isToBag) => {
      ...
      if (user[key] === undefined) item.setter({ count: 0, target: user });
      ...
      msg.msg({
        title: `Действие с сумка ${msg.author.tag}`,
        delete: 9000,
        description,
      });
    };
    ...
    if (action && count) {
      ...
      const itemName = (interaction.params = interaction.params
        .trim()
        .toLowerCase());
