...
export const grempen_products = createCollectionWithKey( [
	{
		...
		fn( boughtContext ) {
			...
			if ( userData.monster ) {
				...
				const count = Math.ceil( randomWith( MIN , max ) )
	...
	} ,
	{
		...
		fn( boughtContext ) {
			...
			if ( userData.iq === undefined ) {
				userData.iq = randomWith( 27 , 133 )
			}

			const value = randomWith( 3 , 7 )
			addResource( {
				user: interaction.user ,
				value ,
	...
	} ,
	{
		...
		others: [ 'идея' , 'идею' ] ,
		fn() {
			const idea = randomElementFromArray( [
			...
			const but = randomElementFromArray( [
	...
	} ,
	{
		...
		fn( boughtContext ) {
			...
			const resource = randomElementFromArray(
				[
					'void' ,
					'seed' ,
					'coins' ,
