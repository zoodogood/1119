...
class Remover {
	...
	getEmbedTitle() {
		...
		case removeStatus === RemoveStatus.Ended:
			return `Удалено ${ ending( removedCount , 'сообщени' , 'й' , 'е' , 'я' ) }!`

		case !!removedCount:
			return `Пожалуйста, Подождите... ${ removedCount } / ${ fetchedMessages.length }`

		default:
			return `Пожалуйста, Подождите... ${ ending(
	...
	async start_remove() {
		...
		while ( byOneDelete.length ) {
			...
			await this.removeMessages( {
				messages: byOneDelete.splice( 0 , randomWith( 5 , 15 ) ) ,
				channel ,
				isBulk: false ,
			} )
			this._interface.updateMessage()
...
class Logger {
	...
	getEmbedDescription() {
		...
		return `${ contents.channel }\n${ contents.user }\n${ contents.label }: ${ capitalize( contents.mode ) }${ contents.canceled }`
	}

	getEmbedTitle() {
		return `Удалено ${ ending( this.remover.removedCount , 'сообщени' , 'й' , 'е' , 'я' ) }`
	}
