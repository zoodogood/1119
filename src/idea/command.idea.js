...
import { fetchMessagesWhile } from '#src/discord/fetchMessagesWhile.js'
import { MessageInterface } from '#src/discord/MessageInterface.js'
import { Pager } from '#src/discord/Pager.js'
import { awaitUserAccept , jsonFile } from '#src/discord/utils.js'
import { disposableListen } from '#src/EventEmitter/export.js'
import {
	around ,
	match ,
	randomWith ,
	zeroCenteredSequence ,
} from '#src/safe-utils.js'
import { TimedCache } from '#src/TimedCache.js'
...
function parseIdeaNumber( authorField ) {
	...
	return +match( authorField.name , /#\d+$/ )?.slice( 1 ) || 0
}
...
class Store extends TimedCache {
	...
	idea_parse_metadata( message ) {
		...
		const reactions = message.reactions.cache.map( ( reaction ) => {
			...
			return { count , emoji: codeOfEmoji( emoji ) }
		} )
		return {
			index ,
			content ,
...
class At_Flagsubcommand {
	...
	messageOptionsOfCurrent() {
		...
		if ( !idea ) {
			...
			const index = ( () => {
				const generator = zeroCenteredSequence()
...
class Command extends BaseCommand {
	...
	async process_start_thread( context , message , phrase ) {
		...
		const { askTheAI_or_null } = await import( '#src/askTheAIOrNull.js' )
		const thread = await message.startThread( {
			name: randomElementFromArray( [
