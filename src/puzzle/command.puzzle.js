...
class Command extends BaseCommand {
	...
	async onChatInput( msg , interaction ) {
		...
		let last = String( randomWith( 1 , 1 ) )
		...
		while ( i > 0 ) {
			...
			while ( last ) {
				...
				ctx.fillRect( randomWith( canv.width ) , randomWith( canv.height ) , 2 , 2 )
				ctx.fillRect( randomWith( canv.width ) , randomWith( canv.height ) , 1 , 1 )
				ctx.fillStyle = 'rgba(242, 250, 250, 0.5)'
				ctx.fillRect( randomWith( canv.width ) , randomWith( canv.height ) , 3 , 3 )
			}
