...
import { template_modules_scope } from '#src/VirtualMachine/template_modules/mod.js'
	...
	processMacroses() {
		...
		while ( _i < _LIMIT ) {
			...
			this.regular = this.regular.replace( regex , ( full , macro , value ) => {
				...
				if ( !macroBase ) {
					return use_unique_characters_marker( full , 'not_macro' , 'g' ).value
				}
		...
		this.regular = this.regular.replaceAll(
			use_unique_characters_marker( '' , 'not_macro' , 'g' ).regex ,
			( full , value ) => value ,
		)
	}
