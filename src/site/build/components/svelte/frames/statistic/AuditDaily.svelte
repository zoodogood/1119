<script>
	...
	class ComponentSectionManager {
		...
		valuestToContent( values ) {
			...
			const content = Object.entries( values )
				.map(
					( [ key , value ] ) =>
						`${ Component.auditTypeEnum.get( key ).label }: ${ NumberFormatLetterize( value ) }` ,
				)
	...
	const Component = {
		...
		async GetData() {
			...
			const entries = Object.entries( data ).map( ( [ day , data ] ) => [
				day * 86_400_000 ,
				data ,
			] )
		...
		async heatMapOnClick( clickEvent ) {
			...
			if ( target.tagName === 'rect' ) {
				...
				const { target: clickTarget } = await new Promise( resolve =>
	...
	const HeatmapState = {
		...
		data: [] ,
		monthLabels: [ ... Array.from( { length: 12 } ) ].map( ( _ , i ) =>
			new Intl.DateTimeFormat( svelteApp.lang , { month: 'short' } ).format(
				dayjs().set( 'month' , i ).toDate() ,
			) ,
		) ,
		dayLabels: [ ... Array.from( { length: 7 } ) ].map( ( _ , i ) =>
			new Intl.DateTimeFormat( svelteApp.lang , { weekday: 'short' } ).format(
				dayjs()
					.set( 'day' , i + 1 )
					.toDate() ,
			) ,
	...
	$: if ( State.data ) {
		HeatmapState.startDate ||= dayjs( [ ... State.data.keys() ].at( 0 ) ).toDate()

		HeatmapState.endDate ||= dayjs( [ ... State.data.keys() ].at( -1 ) ).toDate()

		const key = [ ... Component.auditTypeEnum.keys() ].at(
	...
	$: {
		( ( index ) => {
			State.mainColor = [ ... Component.auditTypeEnum.values() ].at(
				index ,
			).colorTheme
			const MIN_ALPHA = 3
			const toColor = ( _ , i ) => {
			...
			HeatmapState.colors = [ ... Array.from( { length: 16 - MIN_ALPHA } ) ].map( toColor )
		} )( State.selectedAuditTypeIndex )
	}
