...
import { Elements , elementsEnum } from '#src/thing/command.thing.js'

import { ActionsMap } from '#src/user/actions/actionsMap.enum.js'
import UserEffectManager , {
	EffectInfluenceEnum ,
} from '#src/user/actions/EffectsManager.js'
...
export const eventBases = createCollectionWithKey( [
	{
		...
		callback: ( context ) => {
			...
			update_attack_cooldown( user , boss , 'attack.events.increaseAttackCooldown' , context , MINUTE * 20 )
		} ,
	...
	} ,
	{
		...
		callback: ( { user , boss , channel } ) => {
			const hard = ( randomWith( boss.level ) > 20 ) + ( randomWith( boss.level ) > 50 )
	...
	} ,
	{
		...
		callback: ( { user , boss , channel } ) => {
			for ( let i = 0; i < randomWith( 2 , 3 ); i++ ) {
				const hard = ( randomWith( boss.level ) > 20 ) + ( randomWith( boss.level ) > 50 )
				const curse = CurseManager.generate( {
					user ,
					hard ,
	...
	} ,
	{
		...
		callback: async ( context ) => {
			...
			( await CommandsManager.commandInstance( 'grempen' ) ).onChatInput(
				null ,
				Object.assign(
				...
				const spellsTable = {
					...
					'🍯🩸🩸': {
						...
						callback: async ( message , embed ) => {
							...
							const event = randomElementFromArray( pull , {
	...
	} ,
	{
		...
		callback: ( primary ) => {
			...
			const curseBase = cursesBase.get( 'coinFever' )
