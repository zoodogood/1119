...
export function match( string = '' , regular , flags ) {
	const reg = new RegExp( regular , flags )
	const find = string.match( reg )
	return find ? find[ 0 ] : null
}

export function joinWithAndSeparator( arr , ignore = false ) {
...
export function timestampToDate( ms , max ) {
	if ( isNaN( ms ) ) {
		return Number.NaN
	}
...
export function weekHour( ms , locale = 'ru-ru' ) {
	...
	return new Intl.DateTimeFormat( locale , {
		hour: 'numeric' ,
		minute: 'numeric' ,
		weekday: lessThanDay ? 'short' : undefined ,
	} ).format( ms )
}
...
export function similarity( target , compare ) {
...
export function getSimilar( strokes , target ) {
	if (
...
export function NumberFormatLetterize( numb ) {
...
export function resolveDate( day , month , year ) {
	const date = ( new Date )
	if ( day ) {
		date.setDate( day )
	}
...
export function parseDocumentLocate( location ) {
...
export function chunkBySize( array , size ) {
	return makeArray(
		index => array.slice( index * size , index * size + size ) ,
