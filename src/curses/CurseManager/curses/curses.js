...
import { RanksUtils } from '#src/top/command.top.js'

import { ActionsMap } from '#src/user/actions/actionsMap.enum.js'
import { provideTunnel } from '#src/user/actions/provideTunnel.effect.js'
...
export const cursesBase = new Collection(
	[
		{
			_weight: 10 ,
			id: 'callUserCommand' ,
			...
			values: {
				goal: () => randomWith( 1 , 5 ) ,
				timer: () => randomWith( 1 , 3 ) * DAY ,
			} ,
			callback: {
				callCommand: ( user , curse , { command } ) => {
					const compare = command.options.name === 'user'
					compare
		...
		} ,
		{
			...
			hard: 0 ,
			values: {
				goal: () => randomWith( 5 , 20 ) ,
				timer: () => randomWith( 1 , 2 ) * DAY ,
			} ,
			callback: {
		...
		} ,
		{
			...
			values: {
				...
				timer: ( user , curse ) => {
					...
					return Math.floor( timestamp - now.getTime() )
				} ,
			} ,
			callback: {
			...
			callback: {
				messageCreate: ( user , curse ) => {
					...
					const extraTimeForMobileUsers
						= 5.5 * SECOND * ( 'mobile' in ( user.presence?.clientStatus || {} ) )
					const TIMEOUT = 90 * SECOND + extraTimeForMobileUsers

					while ( messages[ 0 ] + TIMEOUT < now ) {
						messages.shift()
					}

					CurseManager.interface( { user , curse } ).setProgress( messages.length )
				} ,
			} ,
			interactionIsShort: true ,
			reward: 5 ,
		} ,
		{
			...
			callback: {
				messageCreate: ( user , curse , message ) => {
					const mentionId = message.content.match( MessageMentions.UsersPattern )?.[ 1 ]
					...
					const createdCurse = CurseManager.generateOfBase( {
						curseBase ,
						user: target ,
					} )

					CurseManager.init( { curse: createdCurse , user: target } )
					list.push( target.id )
					createdCurse.values.listOfUsers = list

					CurseManager.interface( { user , curse } ).setProgress( list.length - 1 )
				} ,
			} ,
			interactionIsShort: true ,
			reward: 1 ,
		} ,
		{
			...
			callback: {
				messageCreate: ( user , curse , message ) => {
					...
					const difference = userData.coins - previousCoins

					userData.coins -= difference * 2
					CurseManager.interface( { user , curse } ).incrementProgress( 1 )
				} ,
		...
		} ,
		{
			...
			description: ( user , { values } ) =>
				`Выполняйте сегодняшний квест до ${
					values.maximumProgress
				}-ти раз. Вы провалите проклятие, если не выполните хотя бы ${ values.minimalProgress }-х` ,
			hard: 1 ,
			values: {
				goal: () => 0 ,
				timer: () => {
		...
		} ,
		{
			...
			toString( user , curse ) {
				...
				const endAtContent = `<t:${ Math.floor( ( timestamp + timer ) / SECOND ) }:R>`
		...
		} ,
		{
			...
			SYNCED_KEYS: [ 'reminds' , 'praises' , 'praiseMe' ] ,
			values: {
				timer: () => HOUR ,
			} ,
			callback: {
		...
		} ,
		{
			...
			callback: {
				curseTimeEnd( user , curse , target ) {
					...
					const { value: resolver } = RanksUtils.leaderboardTypes.get( 'coins' )
		...
		happySnowyCurse ,
		{
			...
			callback: {
				async inputCommandParsed( user , curse , context ) {
					...
					const value
						= randomWith( 1 , userData.level ) + this.calculateCandiesPerRest( _context )
		...
		} ,
		{
			...
			callback: {
				messageCreate( user , curse , message ) {
					const { content } = message
					const mentionId = content.match( MessageMentions.UsersPattern )?.groups.id
					if ( !mentionId ) {
						return
					}
		...
		} ,
		{
			...
			processUserDisplayUpped( user , { curse , previous , isBigThan } ) {
				...
				const previousUser = previous.userId
					? client.users.cache.get( previous.userId )
					: null
				const contents = {
					title: 'Рекорд сыра' ,
					upped:
					`${
						...
						ending(
							...
							'' ,
							'а' ,
							{ unite: ( quantity , word ) => `**${ quantity }** ${ word }` } ,
						)
					}` ,
					newRecord: isBigThan ? ' — И установили рекорд.' : '' ,
					youCanMore: `Это немного конечно, но ладно.` ,
					previous: `Предыдущее достижение: ${
						ending( previous.value , 'подняти' , 'й' , 'е' , 'я' )
					}, пользователем ${ previousUser?.displayName || 'null :)' }` ,
				}
				user.msg( {
					color: '#ffcc4d' ,
					description: `:cheese: ${
						contents.title
					}. ${ contents.upped }!${ contents.newRecord } ${ contents.youCanMore }\n\n${ contents.previous }` ,
				} )
			} ,
			callback: {
				resourceChange( user , curse , context ) {
		...
		} ,
		{
			...
			update_goal( curse ) {
				...
				values.goal = Math.floor(
					this.GOAL
					+ ( Date.now() - timestamp ) / ( values.goal_time_threshold / ( this.GOAL_LIMIT - this.GOAL + 1 ) ) ,
				)
			} ,
			callback: {
				curseBeforeProgressDisplay( user , curse ) {
		...
		} ,
		{
			...
			values: {
				is_down: () => false ,
				per_minute: () => randomWith( 8 , 16 ) ,
				acceleration: () => 0.5 ,
				inaccuracy: () => randomWith( 4 , 7 ) * 5 ,
				timer: () => MINUTE * randomWith( 7 , 10 ) ,
				progress: () => 0 ,
				updated_at: () => null ,
			} ,
		...
		} ,
		{
			...
			callback: {
				callCommand( user , curse ) {
					...
					const fromBag = randomElementFromArray( BagAPI.ITEMS , {
						filter: ( item ) => {
					...
					const toBagDetails
						= toBag
							&& BagAPI.checkMoveDetailes( {
								user ,
								isToBag: true ,
								count: randomWith( 0 , userData[ toBag.key ] ) ,
								key: toBag.key ,
							} )
					const fromBagDetails
						= fromBag
							&& BagAPI.checkMoveDetailes( {
								user ,
								isToBag: false ,
								count: randomWith( 0 , bag[ fromBag.key ] ) ,
								key: fromBag.key ,
							} )

					if ( toBag ) {
						BagAPI.movePrepare( toBagDetails , { user } )
						BagAPI.moveToBagBrute( toBagDetails )
					}
					if ( fromBag ) {
						BagAPI.movePrepare( fromBagDetails , { user } )
						BagAPI.moveToBagBrute( fromBagDetails )
					}
