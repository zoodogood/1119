...
class Command extends BaseCommand {
				...
				[
					{
						action: async (context) => {
							...
							if (userData.chilli && !random(5)) {
								const sellingCount =
									Math.min(userData.chilli, 3 + userData.elementLevel) ?? 0;
								const price = random(sellingCount * 160, sellingCount * 190);
								addResource({
									user,
									value: price,
									executor: user,
			...
			variability: [
				...
				[
					...
					},
					{
						action: async (context) => {
							const { user } = context;
							const isBerrysCountIncreased = random(1);
				...
				[
					...
					false,
					{
						action: async (context) => {
							...
							const bonuses = Math.ceil(berrys * random(1.2, 1.4));
							addResource({
								user,
								value: -berrys,
								executor: user,
				...
				[
					...
					false,
					{
						action: async (context) => {
							...
							const voidCount = (scene.voids = random(1, 2));
							addResource({
								user,
								value: -5,
								executor: user,
					...
					false,
					{
						action: async (context) => {
							...
							collector.on("collect", (message) => {
								!random(25) && addCoinFromMessage(message);
							});
						},
				...
				[
					{
						action: async ({ scene }) => {
							const value = Math.floor(
								random(55, 110) + DataManager.data.bot.berrysPrice / 10,
							);
	...
	async run({ user, elementBase, channel, level, interaction }) {
		...
		const coefficient = random(this.constructor.BASIC_COINS_COEFFICIENT, {
			round: false,
		});
		const scene = {};
		const context = {
