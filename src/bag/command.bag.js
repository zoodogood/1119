	...
	parseCli( params ) {
		const parser = ( new CliParser )
			...
			.captureByMatch( {
				name: 'action' ,
				regex:
					/взять|take|get|[tg]|положить|put|set|[ps]|использовать|use|використати|u/i ,
			} )
			.captureByMatch( {
				name: 'count' ,
				regex: /\d+|\+/ ,
...
class Clean_FlagSubcommand extends BaseFlagSubcommand {
	...
	onProcess() {
		...
		interaction.msg( {
			description: `Очищено ${ ending( toClean.length , 'свойств' , '' , 'о' , 'а' ) }` ,
		} )
	}
}
...
export const ITEMS = [
	{
		...
		onUse( { context } ) {
			...
			const contents = {
				...
				endFlip: randomWith( 1 ) ? 'Орёл' : 'Решка' ,
				result: `` ,
			}
...
class Command extends BaseCommand {
	...
	async onActionUseItem( context ) {
		...
		interaction.channel.msg( {
			description: `${ ending( used , 'единиц' , '' , 'а' , 'ы' ) } предмета ${ emoji } — использовано\n${ phrase }` ,
		} )
	}
	...
	processItemIsExists( context ) {
		...
		const similarItem = getSimilar( list , itemRaw )
		channel.msg( {
