...
import { escapeRegexp , match , randomWith , TimeAuditor } from '#src/safe-utils.js'
...
class TaskGenerator {
	...
	mode() {
		return randomElementFromArray( Object.values( ModesEnum ) , {
	...
	stroke() {
		...
		const count = randomWith( average / 1.2 , average * 1.2 )
		const stroke = [
			... this.task.stickSymbol().repeat( count ) ,
			... '  '.repeat( randomWith( count / 5 ) ) ,
			... '+'.repeat( randomWith( 1 ) ) ,
			... '*'.repeat( randomWith( 1 ) ) ,
			... '/'.repeat( randomWith( 5 ) ? 0 : 1 ) ,
			... '-'.repeat( randomWith( 1 ) ) ,
			... '%'.repeat( randomWith( 1 ) ) ,
			... ','.repeat( randomWith( 1 ) ) ,
			... '.'.repeat( randomWith( 5 ) ? 0 : 1 ) ,
			randomWith( mode === ModesEnum.BitsOperations ? 1 : 0 ) ? '&' : null ,
			randomWith( mode === ModesEnum.BitsOperations ? 1 : 0 ) ? '|' : null ,
			randomWith( mode === ModesEnum.BitsOperations ? 1 : 0 ) ? '~' : null ,
			randomWith( mode === ModesEnum.RomanNumerals ? count / 10 + 2 : 0 )
				? 'V'
				: null ,
			randomWith( mode === ModesEnum.RomanNumerals ? count / 12 + 1 : 0 )
				? 'X'
				: null ,
			randomWith( mode === ModesEnum.RomanNumerals ? count / 12 + 1 : 0 )
				? 'L'
				: null ,
			randomWith( mode === ModesEnum.RomanNumerals ? 1 : 0 ) ? 'C' : null ,
			randomWith( mode === ModesEnum.BooleanOperators ? 1 : 0 ) ? '&&' : null ,
			randomWith( mode === ModesEnum.BooleanOperators ? 1 : 0 ) ? '||' : null ,
			randomWith( mode === ModesEnum.BooleanOperators ? 1 : 0 ) ? '<' : null ,
			randomWith( mode === ModesEnum.BooleanOperators ? 1 : 0 ) ? '>' : null ,
			randomWith( mode === ModesEnum.BooleanOperators ? 1 : 0 ) ? '===' : null ,
		...
		return stroke.replaceAll(
			new RegExp( `(?:${ escapeRegexp( charactersToTrim.join( '|' ) ) })` , 'g' ) ,
			() => '' ,
		)
	}
}
