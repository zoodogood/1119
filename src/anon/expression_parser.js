...
function digitOrCorrectBracket( { previousToken , nextToken } ) {
	return (
...
class AnonExpressionLexer {
	static Tokens = {
		...
		Divide: {
			key: 'Divide' ,
			symbol: '/' ,
			validate: digitOrCorrectBracket ,
			type: TokenTypeEnum.Operator ,
			regexp: '\\/' ,
			operatorPriority: 3 ,
			merge: ( { previousToken , nextToken } ) => {
...
class ExpressionMethods {
	...
	calculateResult( expression , context ) {
		...
		if ( task.mode === ModesEnum.JustCount ) {
			return task.stickCount()
		}
		expression = this.cleanExpression( expression , context )
		return AnonExpressionLexer.toDigit( expression )
	}

	cleanExpression( expression , context ) {
		const { currentTask: task } = context
		const stick = getStickSymbolOfTask( task )
		expression = expression.replace( /[\s,.]/g , '' )

		const numerals = new RegExp(
		...
		expression = AnonExpressionLexer.normalizeExpression( expression )
...
export { AnonExpressionLexer , TokenTypeEnum }
